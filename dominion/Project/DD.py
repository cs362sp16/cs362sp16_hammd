...
def oc_test():
    ...
    assert oc.lookup(list()) is None
    oc.add(list(), 0)
    assert oc.lookup(list()) == 0
    ...
    assert oc.lookup_superset(list()) is not None
    ...
    assert oc.lookup_subset(list()) == 0
...
class DD:
    ...
    def old_dd(self, c, r=list(), n=2):
        """Return the failure-inducing subset of C"""

        assert self.test(list()) == dd.PASS
    ...
    def test_mix(self, csub, c, direction):
        if self.minimize:
            (t, csub) = self.test_and_resolve(csub, list(), c, direction)
            if t == self.FAIL:
                return (t, csub)
        ...
        if self.maximize:
            ...
            (tbar, csubbar) = self.test_and_resolve(csubbar, list(), cbar,
                                                    directionbar)
    ...
    def _dd(self, c, n):
        """Stub to overload in subclasses"""
        print("111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111")
        assert self.test(list()) == self.PASS
